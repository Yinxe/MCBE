...
// - 分配/回收经 DP 读改写（RMW）持久化；跨模组共享同一区域时以"世界真值"
//   兜底（目标槽被外部占用则跳过，绝不覆盖他人物品）。
import type { ItemStack } from "@minecraft/server";
import { capacityOf, slotIdToPosition, type RegionLayout } from "../core/layout";
import { allocateSlotId, createRegionMeta, releaseSlotId, type RegionMeta } from "../core/meta";
...
import { readRegionRecord, writeRegionRecord } from "./store";
...
export class StoredRegion {
   ...
   * - 物化/写入失败（通常为区块未就绪或新桶容器暂不可用）→ 槽位回归空闲池，
  ...
  put(item: ItemStack | undefined): number | null {
    ...
    for (let attempt = 0; attempt < MAX_ALLOC_RETRY; attempt++) {
      ...
      const slotId = allocateSlotId(record.meta, capacityOf(layout));
      ...
      if (!this.runtime.ensureBarrel(pos)) {
        releaseSlotId(record.meta, slotId);
        this.writeRecord(record);
        return null; // 区块未就绪：槽回归空闲池，下次重试
      }
      if (this.runtime.isSlotOccupied(pos)) continue; // 世界已占用 → 丢弃该候选
      if (!this.runtime.writeItem(pos, item)) {
        releaseSlotId(record.meta, slotId);
        this.writeRecord(record);
        return null; // 新桶容器暂不可用：槽回归空闲池，下次重试
      }
      return slotId;
    }
    return null;
