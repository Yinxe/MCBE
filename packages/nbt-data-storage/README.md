...
空洞数据**按层分键**持久化（`nds:item:{key}:pool:{层}`），且存的是 **level-local 索引（0..6911）**，而不是全局 slotId：

- **单值有界**：一层最多 6912 槽 → 单键最坏 ≈ 24KB，稳在 DynamicProperty 单值上限（~32k）内；
- **层数再多也安全**：即使 `maxLevels=64`（容量 442368 槽、slotId 可达 6 位数），落盘的数字也始终 ≤ 4 位——全局 slotId = `level × 6912 + local` 用时现算，**存的时候永远不出现大数字**；
...
| `nds:item:{区域键}`           | 区域主记录：`{ v, dimensionId, layout, meta }`（meta = 水印 + 洞层索引 + 洞数，很小） |
| `nds:item:{区域键}:pool:{层}` | 该层空洞池（JSON level-local 索引数组，单值 ≤ 一层 6912 槽）                          |
