...
import { serializeContainer, serializeEquipment, captureExperience } from "../adapters/McItemCodec";
...
export function saveBotFullState(bot: Player, record: BotRecord): void {
  ...
  // 否则空背包会覆盖持久化的真实数据
  if (!botRegistry.isRestored(record.name)) {
  ...
  if (inv?.container) {
    botStore.saveInventory(record.name, serializeContainer(inv.container));
  }
