...
//
// 劫掠信号（raidStarted / raidVictory）定义在 mock-player 的 raidEvents.ts，属假人模块私有。
...
import { syncEntityTags, TAG_RAID_MODE } from "./core/tags";
import { BotRecord } from "./core/types";
import { raidStarted, raidVictory } from "./raidEvents";
import type { RaidVictoryEvent } from "./raidEvents";
...
/** 袭击之兆（村庄转化，30 秒后触发袭击） */
const RAID_OMEN = "minecraft:raid_omen";
/** 村庄英雄（袭击胜利获得）——劫掠结束判定 */
const VILLAGE_HERO = "minecraft:village_hero";

/** 饮用完整时长（tick）：不祥之瓶需按住 ~1.6s 才消耗完 */
const DRINK_DURATION = 32;

/** 袭击未触发卡死提醒：1 分钟 = 1200 tick（带不祥之兆却久未触发 → 提醒玩家） */
const RAID_STUCK_TICKS = 1200;
