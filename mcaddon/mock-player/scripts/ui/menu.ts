...
function showOperationPanel(player: Player, botName: string): void {
  ...
  form.button(hasControl ? "§6◆ 控制模式 §a[开启]" : "§7◆ 控制模式 §c[关闭]");

  form.button(record.isSneaking ? "§b◆ 潜行 §a[潜行中]" : "§b◆ 潜行 §7[站立]");

  form.button("§d◆ 标签管理");
  form.button("§c◆ 杀死");
  form.button(record.online ? "§e◆ 下线" : "§e◆ 上线");
  form.button("§4◆ 删除");
  form.button("§7← 返回列表");
