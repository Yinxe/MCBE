...
test("createBinding 初始全未绑定", () => {
  const b = createBinding("2:0:-64");
  assert.equal(b.regionId, "2:0:-64");
  assert.equal(b.inv.length, INVENTORY_SIZE);
  assert.ok(b.inv.every((v) => v === null));
  assert.deepEqual(Object.keys(b.equip), []);
