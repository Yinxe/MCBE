...
- **任务**（tasks/index.ts 工厂 barrel，**可扩展**=新文件+导出一行）：NavigateToTask 范式（start 一次性下发导航绝不重复 → tick 只算距离 → 到达/超时/实体失效，onFinish(outcome) 幂等结束回调）；**FollowTask**（每 10tick 周期重下发跟随移动目标，离线/死亡不结束跨周期存活，投掷期间 pauseFollow 全局暂停——1.3.6 已迁移，全局 followMap 引擎删除）；**VaultTask**（宝库开箱循环状态机，1.3.10）：scan（半径 15 搜索最近宝库）→ navigate（持续看向 + 一次性导航到宝库旁站立点，**r<2 且视线命中也是宝库**）→ interact（block state ominous 识别普通/不详 → 按类型换钥匙 → 交互 → 记录钥匙总量基准）→ wait（**纯事件驱动**）；`/mp:move` 已改用实例任务示范
- **宝库事件驱动循环**（1.3.11，用户规格）：等待阶段无轮询——playerInventoryItemChange 只考虑「钥匙数量被 -1 了」且「-1 后不是空手」+ 交互基准 + 总量减少 → 判定成功 → `workflowVaultOpened` 领域事件 → 通知附近真实玩家 + safeReconnect → onOnline 重新开任务（新实体可重复开同一宝库）；换钥匙/槽间移动/手动拆分绝不误判；stop 走 setTags 统一入口 + 取消任务
