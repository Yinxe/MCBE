...
- **单树砍伐计划（rules/woodcut/ChopPlan）**：按模式编排有序目标——**圆木底→顶**
  （先破除树桩，再向上逐根）+ 挡叶/障碍 blocker + 圆木卡叶清理（原木模式）；
  收集模式追加**整树全部树叶**；附拾取范围
- **砍伐执行（features/flow/woodcutFlow）**：先导航到树附近（树中心）→ 破树桩 →
  向上逐根砍（每根用 **breakBlockAt"直到破坏方块"模式**：看向目标方块中心 +
  持续挖掘到被破坏）；目标**超出挖掘距离**（far）→ **靠近目标方块缩短距离再挖**；
  收集模式再挖掘全部树叶
